...
export function PostsGrid() {
  ...
  useEffect(() => {
    async function fetchPosts() {
      try {
        ...
        if (!response.ok) {
          throw new Error('Yazılar yüklenirken bir hata oluştu');
        }
  ...
  if (loading) {
    return (
      <>
        {/* İskelet yükleme ekranı (Skeleton) */}
    ...
    return (
      <div className="flex items-center justify-center min-h-[400px]">
        <div className="text-center space-y-4">
          ...
          <h3 className="text-lg font-semibold">Makaleler yüklenemedi</h3>
    ...
    <>
      ...
      <div className="mb-8 mt-4">
        ...
        <div className="hidden sm:flex flex-wrap gap-2 items-center">
          ...
          >
            {showDesktopTags ? 'Etiketleri Gizle' : 'Etiketleri Göster'}
          </button>
          <button
            className={`px-4 py-2 rounded-full border text-xs font-medium transition-colors ${selectedTag === null ? 'bg-background text-foreground' : 'bg-primary text-primary-foreground'} border-border`}
            onClick={() => setSelectedTag(null)}
            ...
            >
              <Card className="group h-full min-h-[420px] flex flex-col flex-1 overflow-hidden border-0 bg-gradient-to-br from-background/80 to-background/40 backdrop-blur-sm hover:shadow-xl hover:shadow-primary/5 transition-all duration-500 hover:-translate-y-1">
                  ...
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <div className="absolute top-3 right-3">
                      <Badge variant="secondary" className="text-xs bg-background/90 backdrop-blur-sm">
                        {post.source === 'hashnode' ? <><BookOpen className="h-3 w-3 mr-1" /> Hashnode</> : <><Rss className="h-3 w-3 mr-1" /> Medium</>}
                      </Badge>
                    </div>
                  </div>
                )}
                ...
                <CardHeader className="relative space-y-3 flex-1">
                  {!post.coverImage && (
                    <Badge variant="outline" className="w-fit text-xs bg-primary/5 border-primary/20">
                      {post.source === 'hashnode' ? 'Hashnode' : 'Medium'}
                    </Badge>
                  )}
                  ...
                  <div className="flex items-center justify-between text-xs text-muted-foreground">
                    ...
                    {post.readTimeInMinutes && (
                      <div className="flex items-center gap-1">
                        ...
                        <span>{post.readTimeInMinutes} dk okuma</span>
                      </div>
                    )}
                  </div>
          ...
          <PaginationContent>
            ...
            <PaginationItem>
              <PaginationNext
                ...
                onClick={e => {
                  e.preventDefault();
                  if (currentPage < Math.ceil(posts.length / pageSize)) setCurrentPage(p => p + 1);
                }}
